...
fn main(){
    ...
    for ANY7 in 0u8 ..= 2u8.pow(1)-1 {
        ...
        r[InstAddr::from_fields_b(0, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_ADD;       // ADD
        r[InstAddr::from_fields_b(1, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_LLSHFT;    // SLL  ( shift left logical )
        r[InstAddr::from_fields_b(2, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_SIGNBIT | ALU_OP_SUB;       // SLT  ( set less than )
        r[InstAddr::from_fields_b(3, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_SUBC    | ALU_OP_SUB;       // SLTI ( set less than unsigned )
        r[InstAddr::from_fields_b(4, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_XOR;       // XOR
        r[InstAddr::from_fields_b(5, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_RSHFT;     // SRL  ( shift right logical )
        r[InstAddr::from_fields_b(5, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_RSHFT;     // SRA  ( shift right arithmetic )
        r[InstAddr::from_fields_b(6, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_OR;        // OR
        r[InstAddr::from_fields_b(7, ANY7 != 0, OP_OP)] |= REG_OUT | ALU_OUT | ALU_OUTMOD_NONE    | ALU_OP_AND;       // AND
